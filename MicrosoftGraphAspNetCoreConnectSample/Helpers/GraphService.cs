...
namespace MicrosoftGraphAspNetCoreConnectSample.Helpers
{
    public static class GraphService
    {
        ...
        public static async Task<string> GetGroupsJson(GraphServiceClient graphClient, string email, HttpContext httpContext)
        {
            if (email == null) return JsonConvert.SerializeObject(new { Message = "Email address cannot be null." }, Formatting.Indented);

            try
            {
                // Load user groups profile.
                var memberOf = await graphClient.Users[email].MemberOf.Request().GetAsync();
                return JsonConvert.SerializeObject(memberOf, Formatting.Indented);
            }
            catch (ServiceException e)
            {
                switch (e.Error.Code)
                {
                    ...
                    case "TokenNotFound":
                        ...
                        return JsonConvert.SerializeObject(new { e.Error.Message }, Formatting.Indented);
                    default:
                        return JsonConvert.SerializeObject(new { Message = $"An unknown error has occurred. {e.Error}" }, Formatting.Indented);
                }
            }
        }
